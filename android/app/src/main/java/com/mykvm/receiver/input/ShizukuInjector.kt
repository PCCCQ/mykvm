...
class ShizukuInjector {
     ...
     *
     * Character-producing keys are generated by `KeyCharacterMap.getEvents()`,
     * which is exactly how Android's own `input text` builds them: it fills in
     * the scan code and any modifier presses the character needs.
     *
     * This matters. Hand-built `KeyEvent`s (deviceId -1, scanCode 0) looked
     * correct and were accepted by the platform, but produced no text: measured
     * on device with the IME taken out of the path, `adb shell input text`
     * typed into an EditText while our hand-built events did nothing.
     *
     * Keys that type no character (arrows, escape, function and media keys) fall
     * back to the plain event.
     */
    fun key(keyCode: Int, down: Boolean, metaState: Int): Boolean {
        val generated = characterMapEvents(keyCode, down, metaState)
        if (generated != null) {
            // all() short-circuits, so a rejected event aborts the sequence
            return generated.all { transactInject(it, MODE_ASYNC) }
        }
        return transactInject(plainKeyEvent(keyCode, down, metaState), MODE_ASYNC)
    }

    /**
     * The events the character map produces for [keyCode], or null when the key
     * types nothing and the caller should use a plain event instead.
     */
    private fun characterMapEvents(
        keyCode: Int,
        down: Boolean,
        metaState: Int,
    ): List<KeyEvent>? {
        ...
        // KeyCharacterMap.get returns the code point, or 0 when the key types nothing.
        val character: Int = try {
            map.get(keyCode, metaState)
        } catch (error: Throwable) {
            0
        }
        if (character == 0) {
        ...
        val events = try {
            map.getEvents(charArrayOf(character.toChar()))
        } catch (error: Throwable) {
            null
        } ?: return null

        val wanted = if (down) KeyEvent.ACTION_DOWN else KeyEvent.ACTION_UP
        val matching = events.filter { it.action == wanted }
        ...
        matching.forEach { event ->
            try {
                ...
                // Leave the map's own source in place.
            }
        }
        return matching
    }

    private fun plainKeyEvent(keyCode: Int, down: Boolean, metaState: Int): KeyEvent {
